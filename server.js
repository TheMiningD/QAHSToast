...
async function refreshAccessToken() {
  try {
    ...
    executeQuery(
      'SELECT value FROM settings WHERE `key` = "refreshToken"',
      async (err, results) => {
        if (err) throw err;
        ...
        executeQuery(
          'UPDATE settings SET value = ? WHERE `key` = "refreshToken"',
...
app.get("/findPositionInQueue/:trackId", async (req, res) => {
  try {
    ...
    if (data && data.queue) {
      const trackIndex = data.queue.findIndex(
        (item) => item.id === req.params.trackId
      );
...
async function addToQueue(trackUri, accessToken) {
  try {
    const response = await fetch(
      ...
      {
        //not sure why this needs the full thing, breaks without it
        method: "POST",
...
app.post("/api/order", (req, res) => {
  ...
  const query =
    "INSERT INTO orders (name, order_details, notes) VALUES (?, ?, ?)";
...
app.get("/api/get-order-ready-time", (req, res) => {
  executeQuery(
    'SELECT value FROM settings WHERE `key` = "orderReadyTime"',
    (err, results) => {
      if (err) {
...
app.get("/api/get-order-taking-state", (req, res) => {
  executeQuery(
    'SELECT value FROM settings WHERE `key` = "orderTakingEnabled"',
    (err, results) => {
      if (err) {
...
app.post("/api/toggle-order-taking", (req, res) => {
  ...
  executeQuery(
    ...
    (err, results) => {
      ...
      executeQuery(
        'UPDATE settings SET value = ? WHERE `key` = "orderTakingEnabled"',
        [newValue],
        (err, result) => {
          if (err) {
...
app.post("/api/update-order-ready-time", (req, res) => {
  ...
  executeQuery(
    'UPDATE settings SET value = ? WHERE `key` = "orderReadyTime"',
    [newTime],
    (err, result) => {
      if (err) {
...
app.post("/api/serve-order", (req, res) => {
  ...
  pool.getConnection((err, connection) => {
    ...
    connection.beginTransaction((err) => {
      ...
      connection.query(
        "UPDATE orders SET served_at = NOW() WHERE id = ?",
        [orderId],
        (error, results) => {
          if (error) {
          ...
          connection.query(
            "INSERT INTO served_orders (id, name, order_details, created_at, served_at) SELECT id, name, order_details, created_at, NOW() FROM orders WHERE id = ?",
            [orderId],
            (error, results) => {
              if (error) {
                return connection.rollback(() => {
...
app.get("/api/toast-types", (req, res) => {
  executeQuery(
    "SELECT * FROM toast_types WHERE available = TRUE",
    (err, results) => {
...
app.get("/callback", async (req, res) => {
  ...
  try {
    ...
    executeQuery(
      'INSERT INTO settings (`key`, value) VALUES ("refreshToken", ?) ON DUPLICATE KEY UPDATE value = ?',
...
app.post("/api/add-toast-type", (req, res) => {
  ...
  executeQuery(
    "INSERT INTO toast_types (code, type) VALUES (?, ?)",
    [code, type],
    (err, results) => {
      if (err) {
...
app.post("/api/remove-toast-type", (req, res) => {
  const { id } = req.body;
  executeQuery(
    "UPDATE toast_types SET available = FALSE WHERE id = ?",
    [id],
    (err, results) => {
